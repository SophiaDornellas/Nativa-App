// =========================================================================
// TOUR INTERATIVO DO COLETOR - NATIVA APP
// =========================================================================

const ETAPAS_COLETOR = [
    {
        targetId: "tour-anchor-perfil",
        title: "Olá, Coletor Parceiro! 🍃",
        text: "Bem-vindo à sua central de coletas! Aqui você encontra pedidos de recicláveis por toda a cidade e organiza o seu itinerário de trabalho com total autonomia.",
        isFirst: true
    },
    {
        targetId: "tour-anchor-pedidos-title",
        title: "Pedidos Disponíveis",
        text: "Nesta área aparecem em tempo real todas as solicitações de busca cadastradas pelos geradores e comércios da sua região."
    },
    {
        targetId: "tour-anchor-filtros",
        title: "Filtros Estratégicos",
        text: "Use esses filtros para encontrar coletas por Tipo de Resíduo (papelão, plástico), Volume da carga, Data agendada ou por Região de Belo Horizonte."
    },
    {
        targetId: "tour-anchor-agenda-title",
        title: "Minha Agenda de Trabalho",
        text: "Assim que você aceitar um pedido, ele virá para cá! Aqui você terá a Rota no mapa, botão para Finalizar e receber seu bônus, ou Cancelar caso haja imprevistos.",
        isLast: true
    }
];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Inicia no primeiro acesso
    const tourFeito = localStorage.getItem("nativa_tour_coletor_concluido");
    if (!tourFeito) {
        setTimeout(() => {
            iniciarTourCompleto();
        }, 500);
...
function apresentarPasso(indice) {
    const passo = ETAPAS_COLETOR[indice];
    if (!passo) {
        encerrarTour();
        return;
    }
...
function fecharOuAvancar() {
    indiceAtual++;
    if (indiceAtual < ETAPAS_COLETOR.length) {
        apresentarPasso(indiceAtual);
    } else {
        encerrarTour();
    }
...
function encerrarTour() {
    ...
    localStorage.setItem("nativa_tour_coletor_concluido", "true");
}
...
// Disparado no primeiro pedido que o coletor aceitar na plataforma
window.notificarPrimeiroPedidoAceito = function() {
    const jaAceitou = localStorage.getItem("nativa_primeiro_pedido_aceito");
    if (!jaAceitou) {
        localStorage.setItem("nativa_primeiro_pedido_aceito", "true");
        setTimeout(() => {
            indiceAtual = 3; // Abre a explicação de "Minha Agenda de Trabalho"
            apresentarPasso(indiceAtual);
        }, 600);
    }
};